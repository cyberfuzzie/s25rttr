// $Id: GameWorldGame.cpp 6535 2010-07-03 08:12:55Z FloSoft $
//
...
void GameWorldGame::DestroyFlag(const MapCoord x, const MapCoord y)
{
	...
	if(GetNO(x,y)->GetType() == NOP_FLAG)
	{
		// Dann abreiÃŸen mitsamt GebÃ¤ude
...
///////////////////////////////////////////////////////////////////////////////
/**
 *  setzt den echten StraÃŸen-Wert an der Stelle X,Y (berichtigt).
 *
...
{
	// Gucken, ob das GebÃ¤ude hier überhaupt noch gebaut wrden kann
...
{
	// Steht da auch ein GebÃ¤ude oder eine Baustelle, nicht dass wir aus VerzÃ¶gerung Feuer abreiÃŸen wollen, das geht schief
		...
		GetNO(x,y)->GetType() == NOP_BUILDINGSITE)
	{
		...
		// Ist das GebÃ¤ude auch von dem Spieler, der es abreiÃŸen will?
							  ...
							  unsigned short start_x,unsigned short start_y, const std::vector<unsigned char>& route)
{
	...
	// Evtl Zierobjekte abreiÃŸen (Anfangspunkt)
	...
	// Der Wirtschaft mitteilen, dass eine neue StraÃŸe gebaut wurde, damit sie alles NÃ¤cige macht
...
{
	std::list<nobBaseMilitary*> buildings; // Liste von MilitÃ¤rgebÃ¤uden in der NÃ¤he
	...
	for(std::list<nobBaseMilitary*>::iterator it = buildings.begin();it!=buildings.end();++it)
	{
		...
		// Wenn das GebÃ¤ude abgerissen wird oder wenn es noch nicht besetzt war, natürlich nicht mit einberechnen
		if(*it != building || !destroyed)
			tr.CalcTerritoryOfBuilding(*it);
	}

	// Baustellen von HÃ¤fen mit einschlieÃŸen
	...
	for(int y = y1;y<y2;++y)
	{
		for(int x = x1;x<x2;++x)
		{
			...
			// nicht das MilitÃ¤rgebÃ¤ude oder dessen Flagge nochmal abreiÃŸen
			if(owner_changed[(x2-x1)*(y-y1)+(x-x1)])
			{
				for(unsigned char i = 0;i<6;++i)
				{
					...
					// ggf den noch darüber, falls es eine Flagge war (kann ja ein GebÃ¤ude entstehen)
...
void GameWorldGame::DestroyPlayerRests(const MapCoord x, const MapCoord y, const unsigned char new_player,const noBaseBuilding * exception)
{
	...
	// Flaggen, GebÃ¤ude und Baustellen zerstÃ¶ren, aber keine übernommenen und nicht die Ausahme oder dessen Flagge!
	...
	if(flag)
	{
		// Die MinistraÃŸe von dem MilitÃ¤rgebÃ¤ude nich abreiÃŸen!
...
bool GameWorldGame::IsNodeForFigures(const MapCoord x, const MapCoord y)
{
	...
	for(unsigned char i = 0;i<6;++i)
	{
		...
		else if(TERRAIN_BQ[t] == BQ_DANGER) return false; // in die NÃ¤he von Lava usw. dürfen die Figuren gar nich kommen!
	}
...
void GameWorldGame::Attack(const unsigned char player_attacker, const MapCoord x, const MapCoord y, const unsigned short soldiers_count, const bool strong_soldiers)
{
	...
	// Existiert das angegriffenen GebÃ¤ude überhaupt noch?
	...
	// Prüfen, ob der angreifende Spieler das GebÃ¤ude überhaupt sieht (Cheatvorsorge)
	...
	for(std::list<nobBaseMilitary*>::iterator it = buildings.begin();it!=buildings.end();++it)
	{
      ...
      {
        // je einen soldaten zum entfernen vormerken für jeden EXTENDED_ATTACKING_DISTANCE groÃŸen Schritt
			...
			if(soldiers_count)
			{
				...
				if(FindHumanPath(x,y,(*it)->GetX(),(*it)->GetY(),MAX_ATTACKING_RUN_DISTANCE) != 0xFF) // TODO check: hier wird ne random-route berechnet? soll das so?
				{
						...
						;it2.valid()&&i<soldiers_count;++i)
					{
						...
						{
							// Ist das einzufügende Item grÃ¶ÃŸer als das aktuelle?
...
bool GameWorldGame::IsRoadNodeForFigures(const MapCoord x, const MapCoord y,const unsigned char dir)
{
	...
	for(list<noBase*>::iterator it = objects.begin();it.valid();++it)
	{
		...
		if((*it)->GetType() == NOP_FIGURE)
		{
			...
			// AuÃŸerdem muss auch die Position stimmen, sonst spinnt der ggf. rum, da 
...
{ 
	...
	{
		// Ist SpÃ¤turm überhaupt besetzt?
		if(!(*it)->HasWorker())
			continue;
...
bool GameWorldGame::IsScoutingFigureOnNode(const MapCoord x, const MapCoord y, const unsigned player, const unsigned distance) const
{
	...
	// SpÃ¤her/Soldaten in der NÃ¤he prüfen und direkt auf dem Punkt
	for(list<noBase*>::iterator it = objects.begin();it.valid();++it)
	{
