...
noBaseBuilding * GameClientPlayer::FindClientForWare(Ware * ware)
{
	...
	// Bretter und Steine k�nnen evtl. auch H�fen f�r Expeditionen gebrauchen
	...
	if(!bb)
	{	// Zuerst Einlagernde Lagerh�user durchgehen
		bb = FindWarehouse(ware->GetLocation(),FW::Condition_WantStoreWare,0,true,&gt,true);
		// Wenn das nichts wurde, dann auch restliche Lagerh�user mit einbeziehen
		if(!bb)
...
{
	// Pr�fen, ob Abstand zu bisherigen Punkten nicht zu klein
