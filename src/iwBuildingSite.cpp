// $Id: iwBuildingSite.cpp 4652 2009-03-29 10:10:02Z FloSoft $
//
...
: IngameWindow(buildingsite->CreateGUIID(), 0xFFFE, 0xFFFE, 226, 194, _(BUILDING_NAMES[buildingsite->GetBuildingType()]), GetImage(resource_dat,41)),
	gwv(gwv),buildingsite(buildingsite)
{
	// Bild des Gebäudes
	AddImage(0, 113, 130, GetBobImage(GAMECLIENT.GetLocalPlayer()->nation, 250+5*buildingsite->GetBuildingType()));
	// Gebäudename 
	AddText(1, 113, 44, _("Order of building site"), COLOR_YELLOW, glArchivItem_Font::DF_CENTER, NormalFont);

	// Hilfe
	AddImageButton( 2,  16, 147, 30, 32, TC_GREY, GetImage(io_dat,  21));
	// Gebäude abbrennen
	AddImageButton( 3,  50, 147, 34, 32, TC_GREY, GetImage(io_dat,  23));

	// "Gehe Zu Ort"
	AddImageButton( 4, 179, 147, 30, 32, TC_GREY, GetImage(io_dat, 107), _("Go to place"));
}
