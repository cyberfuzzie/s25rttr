...
void nofWoodcutter::DrawWorking(int x,int y)
{
	...
	if(i < 10)
	{
		...
		GetBobFile(jobs_bob)->Draw(5,0,false,i%8,x-i,y,COLORS[GAMECLIENT.GetPlayer(player)->color]);
	...
	else if(i<115)
	{
		...
		GetBobFile(jobs_bob)->Draw(5,3,false,(i-105)%8,x-(9-(i-105)),y,COLORS[GAMECLIENT.GetPlayer(player)->color]);
		DrawShadow(x-(9-(i-95)),y,static_cast<unsigned char>((i-105)%8),dir);
	}
	else
	{
		// 5. kurz am Baum warten (quasi Baumstamm in die Hand nehmen)
		GetBobFile(jobs_bob)->Draw(5,3,false,1,x,y,COLORS[GAMECLIENT.GetPlayer(player)->color]);
